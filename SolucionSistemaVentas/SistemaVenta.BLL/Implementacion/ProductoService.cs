        ...
        public async Task<List<Producto>> Lista()
        {
            //throw new NotImplementedException();
                ...
                return query.Include(c => c.IdCategoriaNavigation).ToList();
                                                        ...
                                                        //, Stream imagen = null, string NombreImagen="" -- lo elimine 
        public async Task<Producto> Crear(Producto entidad, Stream imagen = null, string NombreImagen = "")
        {
                                                                                             
            Producto producto_existe = await _repositorio.Obtener(p => p.Descripcion == entidad.Descripcion);
            ...
            try
            {
                ...
                if(producto_creado.IdProducto == 0)
                    throw new TaskCanceledException("No se pudo crear el producto amigue");
            ...
            }catch(Exception ex)
            {
                throw;
                                                                               ...
                                                                               //, string NombreImagen = ""  --- falta agregar
        public async Task<Producto> Editar(Producto entidad, Stream imagen = null, string NombreImagen = "")
        {                                                             
            Producto producto_existe = await _repositorio.Obtener(p => p.Descripcion == entidad.Descripcion && p.IdProducto != entidad.IdProducto);
            ...
            try
            {
               ...
               if(producto_para_editar.NombreImagen == "")
                ...
                if (!respuesta) 
                    throw new TaskCanceledException("No se pudo editar el producto amigue");
